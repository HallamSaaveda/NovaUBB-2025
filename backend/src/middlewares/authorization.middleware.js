...
export async function isAdmin(req, res, next) {
  ...
  try {
    ...
    // Permitir acceso tanto a admin como a superadmin
    if (rolUser === "admin" || rolUser === "superadmin") {
      //? If the user is an administrator or superadministrator, call the next middleware.
      next()
      return
    }
...
export function verifyRole(requiredRoles) {
  //* Function that verifies the role of the user.
  return (req, res, next) => {
    ...
    if (!roles.includes(user.role)) {
      //? If the user's role is not in the roles array, return an error message.
      return res.status(403).json({ message: "No tienes permisos para acceder a esta ruta." })
    }
